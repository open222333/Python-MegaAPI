# ============================================================
# 20251110 無法使用 Mega API 登入功能，暫時停用此檔案
# ============================================================
...
class Mega:
    ...
    def mega_login(self, email, password):
        ...
        if isinstance(result, dict) and "csid" in result:
            self.logger.debug(f"登入成功！Session ID: {result['csid']}")
            return result
        else:
    ...
    def list_files(self):
        ...
        if isinstance(result, dict) and "f" in result:
            files = result["f"]
            self.logger.debug(f"共取得 {len(files)} 個檔案/資料夾。")
            return files
        else:
    ...
    def upload_file(self, file_path, folder_node=None):
        ...
        upload_url = result
        self.logger.debug(f"取得上傳 URL: {upload_url}")

        # 進行實際上傳
        with open(file_path, "rb") as f:
            upload_res = requests.post(upload_url, data=f.read())
        if upload_res.status_code == 200:
            self.logger.debug(f"檔案上傳成功: {file_name}")
            return True
        else:
            self.logger.error(
    ...
    def delete_file(self, node_id):
        ...
        if result == 0:
            self.logger.debug(f"檔案刪除成功：{node_id}")
            return True
        else:
